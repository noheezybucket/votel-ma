...
class StatisticsController extends Controller
{
    function index(Request $request)
    {
        ...
        $likes = Like::pluck('program_id', 'like');

        // return $candidats;

        $chart1 = new CandidatesNumber;
        $chart1->labels($likes->keys());
        $chart1->dataset('Likes', 'bar', $likes->values())->backgroundColor(['#F42B03', '#1B2CC1', '#1EFFBC']);

        $chart2 = new CandidatesNumber;
        $chart2->labels($programmes->keys());
        $chart2->dataset('Programmes', 'doughnut', $programmes->values())->backgroundColor(['#F42B03', '#1B2CC1', '#1EFFBC']);
